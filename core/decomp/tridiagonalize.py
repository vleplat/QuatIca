...
def householder_vector(a, v):
    ...
    if np.any(np.imag(v) != 0):
        raise ValueError('Parameter v may not be a quaternion vector (mathematical limitation)')
...
def check_tridiagonal(B):
    ...
    if T1 > T2 * tolerance:
        print(f"Warning: Result of tridiagonalization was not accurately tridiagonal.")
    ...
    diag_vector = np.array([np.sqrt(diag_main[i].x**2 + diag_main[i].y**2 + diag_main[i].z**2) 
    ...
    if T2_vector > T1_scalar * tolerance:
        print(f"Warning: Result of tridiagonalization was not accurately scalar.")
        print(f"Largest on-tridiagonal vector and scalar moduli were: {T2_vector}, {T1_scalar}")
