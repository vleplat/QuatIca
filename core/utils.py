...
def power_iteration(A, max_iterations=100, tol=1e-10, return_eigenvalue=False, verbose=False):
    ...
    try:
        if verbose and not ishermitian(A):
            print("[power_iteration] Notice: A appears non-Hermitian; eigenvalue estimate will be real (magnitude-based). For complex eigenvalues, use power_iteration_nonhermitian().")
    except Exception:
...
def quaternion_to_complex_adjoint(A: np.ndarray) -> np.ndarray:
    ...
    Notes:
      - This enables using complex eigenvalue algorithms on quaternion matrices by
        choosing the complex subfield generated by the x-axis unit.
    """
...
def _power_iteration_complex(M: np.ndarray, max_iter: int = 1000, eig_tol: float = 1e-12, res_tol: float | None = 1e-10, seed: int = 0) -> Tuple[complex, np.ndarray, List[float]]:
...
def power_iteration_nonhermitian(A: np.ndarray, max_iterations: int = 5000, eig_tol: float = 1e-12, res_tol: float | None = 1e-10, seed: int = 0, return_vector: bool = True):
    """Power iteration for NON-Hermitian quaternion matrices (experimental).

    Returns a complex eigenvalue estimate (in the fixed complex subfield tied to the x-axis)
    and a corresponding quaternion eigenvector approximation.

    Parameters:
      - A: square quaternion matrix (non-Hermitian allowed)
      - max_iterations: maximum iterations for the complex power iteration
      - eig_tol: tolerance for eigenvalue stabilization
      - res_tol: tolerance on residual ||Mv - lambda v|| to declare convergence
      - seed: RNG seed for initialization
      - return_vector: if True, also return the quaternion eigenvector approximation

    Returns:
      - (q_vec, lambda_complex, residuals) if return_vector is True
      - (lambda_complex, residuals) otherwise
    ...
    Notes:
      ...
      - This routine chooses a fixed complex subfield (x-axis). Eigenvalues appear
        in conjugate pairs in the adjoint; we return one complex root.
    """
    M = quaternion_to_complex_adjoint(A)
    lam, v_c, residuals = _power_iteration_complex(M, max_iter=max_iterations, eig_tol=eig_tol, res_tol=res_tol, seed=seed)
    ...
    q_vec = quaternion.as_quat_array(q_arr).reshape(n,)
    if return_vector:
        return q_vec, lam, residuals
