...
def quaternion_to_complex_adjoint(A: np.ndarray, axis: str = "x") -> np.ndarray:
    ...
    if axis != "x":
        raise NotImplementedError("quaternion_to_complex_adjoint currently supports axis='x' only")
    n = A.shape[0]
...
def power_iteration_nonhermitian(
    ...
    if _is_hermitian_quat(A):
        v_h, lam_mag = power_iteration(A, max_iterations=max_iterations, tol=eig_tol, return_eigenvalue=True)
        ...
        if return_vector:
            return v_h.reshape(A.shape[0],), lam_out, residuals
    ...
    if q_norm > 0:
        q_vec = (q_vec / q_norm).reshape(n,)
    # If A is Hermitian, force eigenvalue to be real (numerical cleanup)
    if _is_hermitian_quat(A):
        lam = complex(float(np.real(lam)), 0.0)
    # Format eigenvalue
    if eigenvalue_format == "quaternion":
        lam_out = quaternion.quaternion(float(np.real(lam)), float(np.imag(lam)), 0.0, 0.0)
    else:
        lam_out = lam
    if return_vector:
