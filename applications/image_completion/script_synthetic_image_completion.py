...
def create_test_image(shape):
    ...
    # Stack channels
    img = np.stack(
        [r[None, :].repeat(h, axis=0), g[:, None].repeat(w, axis=1), b], axis=-1
    )
    return img
...
def MRQ(A, max_iter=25):
    ...
    solver = NewtonSchulzPseudoinverse(
        gamma=1.0, max_iter=max_iter, tol=1e-6, verbose=False
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Parameters: rank_col={rank_col}, rank_row={rank_row}, iterations={n_iter}, missing_rate={mr}"
    )
