...
def MRQ(A, max_iter=25):
    solver = NewtonSchulzPseudoinverse(
        gamma=0.9, max_iter=max_iter, tol=1e-6, verbose=False
    )
...
if __name__ == "__main__":
    ...
    img_path = os.path.join("dataset", "sample_image.jpg")
    ...
    X = B_Miss.copy()
    n_iter = 100
    rank_col = 50  # Smaller rank for smaller image
    rank_row = 50  # Smaller rank for smaller image
    max_it_pinv = 25
    ...
    for i in range(n_iter):
        ...
        # Gaussian blur (denoising) - optimal sigma for best performance
    ...
    plt.title("Masked Image (50% missing)")
    plt.axis("off")
