...
print("\n" + "=" * 60)
print(
    "Alternative: Using actual residual norms (if relative residual calculation is not accurate):"
)
print("=" * 60)
...
for result in results:
    ...
    print(
        f"{N} & NS--Q   & {result['newton']['iterations']:3d} & {result['newton']['time']:6.3f} & {result['newton']['residual']:.1e} \\\\"
    )
    print(
        f"    & QGMRES & {result['qgmres']['iterations']:3d} & {result['qgmres']['time']:6.3f} & {result['qgmres']['residual']:.1e} \\\\"
    )
    print("\\hline")
