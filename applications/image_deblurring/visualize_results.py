...
def create_performance_plots(results):
    ...
    axes[0, 0].plot(sizes, qslst_time_16, 'b-o', linewidth=2, markersize=8, label='QSLST-FFT')
    ...
    axes[0, 1].plot(sizes, qslst_psnr_16, 'b-o', linewidth=2, markersize=8, label='QSLST-FFT')
    ...
    axes[0, 2].plot(sizes, qslst_ssim_16, 'b-o', linewidth=2, markersize=8, label='QSLST-FFT')
    ...
    axes[1, 0].plot(sizes, qslst_time_20, 'b-o', linewidth=2, markersize=8, label='QSLST-FFT')
    ...
    axes[1, 1].plot(sizes, qslst_psnr_20, 'b-o', linewidth=2, markersize=8, label='QSLST-FFT')
    ...
    axes[1, 2].plot(sizes, qslst_ssim_20, 'b-o', linewidth=2, markersize=8, label='QSLST-FFT')
...
def create_summary_table(results):
    ...
    print("\n📊 SUMMARY TABLE")
    print("=" * 80)
    print(f"{'Image':<10} {'Size':<6} {'Method':<12} {'Time(s)':<8} {'PSNR(dB)':<10} {'SSIM':<8}")
    print("-" * 80)
    ...
    for image in ['kodim16', 'kodim20']:
        ...
        for result in sorted(image_data, key=lambda x: x['size']):
            ...
            print(f"{image:<10} {size:<6} {'QSLST-FFT':<12} {qslst['time']:<8.3f} {qslst['psnr']:<10.2f} {qslst['ssim']:<8.3f}")
            print(f"{'':<10} {size:<6} {'FFT-NS-Q':<12} {ns['time']:<8.3f} {ns['psnr']:<10.2f} {ns['ssim']:<8.3f}")
            print("-" * 80)
...
def main():
    ...
    if not os.path.exists(json_file):
        ...
        print("Please run the benchmark first: python applications/image_deblurring/run_deblur_benchmark.py")
        return
