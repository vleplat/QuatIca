...
# Ensure we can import from quatica
...
def main():
    parser = argparse.ArgumentParser(
        description="Quaternion image deblurring benchmark: QSLST (FFT/matrix) vs NS/HON"
    )
    parser.add_argument(
        "--size",
        type=int,
        default=32,
        help="Resize to size×size for all paths (default: 32)",
    )
    parser.add_argument(
        "--lam",
        type=float,
        default=1e-3,
        help="Tikhonov lambda for QSLST (default: 1e-3)",
    )
    parser.add_argument(
        "--snr", type=float, default=None, help="Optional SNR dB (add AWGN)"
    )
    parser.add_argument(
        "--ns_mode",
        type=str,
        default="dense",
        choices=["dense", "sparse", "fftT", "tikhonov_aug"],
        help="NS mode: dense/sparse A^†, fftT for NS on T, or tikhonov_aug for augmented [A;sqrt(lam)I]",
    )
    parser.add_argument(
        "--ns_iters",
        type=int,
        default=14,
        help="Iterations for NS when ns_mode=fftT (default: 14)",
    )
    parser.add_argument(
        "--fftT_order",
        type=int,
        default=2,
        choices=[2, 3],
        help="Order of inverse-NS in fftT mode: 2 (Newton–Schulz) or 3 (Halley)",
    )
    parser.add_argument(
        "--image",
        type=str,
        ...
        help="Image to use: kodim16 or kodim20",
    )
    ...
    rgb = (
        np.asarray(
            Image.fromarray((rgb_full * 255).astype(np.uint8)).resize(
    ...
    def _ns_tikhonov_fft_restore(
        Bq_in: np.ndarray, psf_in: np.ndarray, lam_in: float, iters: int
    ) -> np.ndarray:
        from qslst import _pad_psf  # reuse centering
    ...
    def _hon_ns_tikhonov_fft_restore(
        Bq_in: np.ndarray, psf_in: np.ndarray, lam_in: float, iters: int
    ) -> np.ndarray:
    ...
    if args.ns_mode == "fftT":
        ...
        print(
            f"[NS] Computing T^{-1} via inverse-NS in Fourier domain (order={args.fftT_order}, iters={args.ns_iters}) ...",
            flush=True,
        )
        t1 = time.time()
        ...
        print(
            "[NS] Computing Tikhonov solution via augmented matrix C=[A; sqrt(lam) I] ...",
            flush=True,
        )
        ...
        print(
            f"[NS] Computing A^† via Newton–Schulz ({args.ns_mode}, fast mode) ...",
            flush=True,
        )
        ns_solver = NewtonSchulzPseudoinverse(
        ...
        if hon_skipped:
            print(
                "[HON-NS] Skipped in sparse mode (sparse algebra not supported for HON updates)",
                flush=True,
            )
            X_hon = X_ns.copy()
            t_hon = 0.0
            ...
            print(
                "[HON-NS] Computing A^† via Higher-Order Newton–Schulz (quaternion, fast mode) ...",
                flush=True,
            )
            t2 = time.time()
            hon_solver = HigherOrderNewtonSchulzPseudoinverse(
                max_iter=40, tol=0.0, verbose=False
            )
    ...
    save_rgb_image(
        rgb_qslst_mat, os.path.join(out_dir, f"deblur_qslst_matrix_{base_name}.png")
    )
    ...
    print(
        f"  QSLST (FFT):    PSNR={psnr_qslst:.2f}dB  SSIM={ssim_qslst:.3f}  RelErr={rel_qslst:.3e}  time={t_qslst:.3f}s"
    )
    print(
        f"  QSLST (matrix): PSNR={psnr_qslst_mat:.2f}dB SSIM={ssim_qslst_mat:.3f} time={t_qslst_mat:.3f}s"
    )
    print(
    ...
    if hon_skipped:
        ...
        print(
            f"  HON-NS (A^†):    PSNR={psnr_hon:.2f}dB   SSIM={ssim_hon:.3f}   RelErr={rel_hon:.3e}   time={t_hon:.3f}s"
        )
    print(
