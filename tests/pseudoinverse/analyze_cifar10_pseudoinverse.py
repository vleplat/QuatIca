...
def visualize_sample_images(X, labels, class_names, n_samples_per_class=5):
    ...
    fig, axes = plt.subplots(
        n_classes, n_samples_per_class, figsize=(2 * n_samples_per_class, 2 * n_classes)
    )
    ...
    plt.savefig(
        os.path.join(out_dir, "sample_images_verification.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
    ...
    print("  Spatial dimensions: 32x32 pixels")
    print("  Color channels: RGB (from quaternion x,y,z components)")
...
def visualize_class_average_filters(X_pinv, labels, class_names, image_shape=(32, 32)):
    ...
    fig.suptitle(
        "Class-Average Reconstruction Filters (Pseudoinverse Columns)", fontsize=16
    )
    ...
    for cls in range(n_classes):
        ...
        for c in range(4):
            ax = axes[cls, c]
            ax.imshow(
                avg_q_img[:, :, c], cmap="seismic", vmin=global_min, vmax=global_max
            )
            ax.set_title(f"{class_names[cls]}\n{comps[c]}")
            ax.axis("off")
    ...
    cbar = fig.colorbar(
        axes[0, 0].images[0], ax=axes, orientation="horizontal", fraction=0.05, pad=0.1
    )
    ...
    os.makedirs(out_dir, exist_ok=True)
    plt.savefig(
        os.path.join(out_dir, "class_average_filters.png"), dpi=300, bbox_inches="tight"
    )
    plt.show()
    ...
    for cls in range(n_classes):
        ...
        for c, comp_name in enumerate(["Real", "Red", "Green", "Blue"]):
            comp_data = avg_q[:, c]
            print(
                f"  {comp_name}: mean={comp_data.mean():.6f}, std={comp_data.std():.6f}, "
...
def visualize_pca_analysis(X_pinv, labels=None, class_names=None):
    ...
    if labels is not None and class_names is not None:
        ...
        plt.scatter(
            Z[:, 0], Z[:, 1], c="gray", s=30, alpha=0.7, edgecolors="black", linewidth=0.5
        )
    ...
    print(
        f"  Number of components for 90% variance: {np.argmax(cumulative_var >= 0.90) + 1}"
    )
    print(
        f"  Number of components for 99% variance: {np.argmax(cumulative_var >= 0.99) + 1}"
    )
...
def visualize_pseudoinverse_insights(X, X_pinv, labels, image_shape=(32, 32)):
    ...
    fig, axes = plt.subplots(
        len(pixel_positions), 4, figsize=(16, 4 * len(pixel_positions))
    )
    ...
    for idx, (pixel_idx, description) in enumerate(pixel_positions):
        ...
        if len(filter_components) >= 32 * 32:
            ...
            filter_components[: 32 * 32].reshape(32, 32, 4)
        else:
    ...
    plt.tight_layout()
    plt.savefig(
        "../../output_figures/pixel_reconstruction_filters.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(
        "../../output_figures/spectral_analysis.png", dpi=300, bbox_inches="tight"
    )
    plt.show()
    ...
    os.makedirs(out_dir, exist_ok=True)
    plt.savefig(
        os.path.join(out_dir, "pseudoinverse_manifold.png"), dpi=300, bbox_inches="tight"
    )
    plt.show()
    ...
    for i in range(4):
        for j in range(4):
            corr_matrix[i, j] = np.corrcoef(
                comp_pinv[..., i].flatten(), comp_pinv[..., j].flatten()
            )[0, 1]
    ...
    os.makedirs(out_dir, exist_ok=True)
    plt.savefig(
        os.path.join(out_dir, "channel_correlations.png"), dpi=300, bbox_inches="tight"
    )
    plt.show()
    ...
    os.makedirs(out_dir, exist_ok=True)
    plt.savefig(
        os.path.join(out_dir, "class_spectral_analysis.png"), dpi=300, bbox_inches="tight"
    )
    plt.show()
