...
def load_small_images_from_directory(directory_path, target_size=(32, 32), max_images=20):
    ...
    print(
        f"Each row represents a {target_size[0]}x{target_size[1]} image vectorized to {target_size[0] * target_size[1]} quaternions"
    )
...
def visualize_sample_images(X, image_names, image_shape=(32, 32), n_samples=8):
    ...
    for i in range(n_show):
        ...
        spatial_image = float_image.reshape(
    ...
    filename = os.path.join(
        out_dir, f"multiple_images_sample_verification_{timestamp}.png"
    )
...
def visualize_image_average_filters(X_pinv, image_names, image_shape=(32, 32)):
    ...
    fig.suptitle(
        "Image-Average Reconstruction Filters (Pseudoinverse Columns)", fontsize=16
    )
    ...
    for i in range(n_show):
        print(
            f"Computing filter for image {image_names[i].split('.')[0]} ({i + 1}/{n_show})"
        )
        ...
        for c in range(4):
            ax = axes[i, c]
            ax.imshow(col_img[:, :, c], cmap="seismic", vmin=global_min, vmax=global_max)
            ax.set_title(f"{image_names[i].split('.')[0]}\n{comps[c]}")
            ax.axis("off")
...
def visualize_pca_analysis(X_pinv, image_names):
    ...
    plt.subplot(1, 2, 2)
    plt.scatter(
        Z[:, 0], Z[:, 1], c="blue", s=50, alpha=0.7, edgecolors="black", linewidth=0.5
    )
    ...
    if len(cumulative_var) > 4:
        print(
            f"  Total variance explained by first 5 components: {cumulative_var[4]:.1%}"
        )
    else:
        print(
            f"  Total variance explained by all {len(cumulative_var)} components: {cumulative_var[-1]:.1%}"
        )
    print(
...
def visualize_advanced_insights(X, X_pinv, image_names, image_shape=(32, 32)):
    ...
    os.makedirs(out_dir, exist_ok=True)
    filename = os.path.join(
        out_dir, f"multiple_images_channel_correlations_{timestamp}.png"
    )
    plt.savefig(filename, dpi=300, bbox_inches="tight")
    ...
    if n_images <= 15:
        plt.xticks(
            range(n_images),
            [name.split(".")[0] for name in image_names],
            rotation=45,
            ha="right",
        )
