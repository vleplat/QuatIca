...
def create_validation_visualization(
    ...
    ax_title.text(
        0.5,
        0.8,
        "QUATERNION PSEUDOINVERSE VALIDATION - PART 1",
        fontsize=24,
        fontweight="bold",
        ha="center",
    )
    ax_title.text(
        0.5,
        0.6,
        "Original Matrix and Theoretical vs Numerical Comparison",
        fontsize=16,
        ha="center",
    )
    ax_title.text(
        0.5,
        0.4,
        "Reference: Huang, L., Wang, Q.-W., & Zhang, Y. (2015)",
        fontsize=12,
        ha="center",
        style="italic",
    )
    ax_title.text(
        0.5,
        0.2,
        "Linear Algebra and its Applications, 475, 45-61",
        fontsize=12,
        ha="center",
        style="italic",
    )
    ...
    ax_title2.text(
        0.5,
        0.8,
        "QUATERNION PSEUDOINVERSE VALIDATION - PART 2",
        fontsize=24,
        fontweight="bold",
        ha="center",
    )
    ax_title2.text(
        0.5, 0.6, "Error Analysis and Convergence History", fontsize=16, ha="center"
    )
    ...
    diff = np.abs(
        quaternion.as_float_array(X_small) - quaternion.as_float_array(A_theoretical)
    )
    ...
    ax_diff.set_title(
        "Absolute Difference\n|Theoretical - Numerical| (w-component)",
        fontweight="bold",
        fontsize=18,
    )
    ...
    ax_conv.axhline(
        y=1e-6, color="g", linestyle=":", linewidth=2, label="Tolerance (1e-6)"
    )
    ...
    ax_title3.text(
        0.5,
        0.8,
        "QUATERNION PSEUDOINVERSE VALIDATION - PART 3",
        fontsize=24,
        fontweight="bold",
        ha="center",
    )
    ax_title3.text(
        0.5,
        0.6,
        "Component-wise Analysis: Theoretical vs Numerical",
        fontsize=16,
        ha="center",
    )
    ...
    ax_comp1.bar(
        x_pos + width / 2, num_flat[0], width, label="Numerical", alpha=0.8, color="red"
    )
    ...
    for i, comp in enumerate(components):
        ...
        ax_comp1.text(
            i,
            max(theo_flat[0, i], num_flat[0, i]) + 0.01,
            f"Δ={diff_val:.2e}",
            ha="center",
            fontsize=12,
            color="red",
    ...
    ax_comp2.bar(
        x_pos + width / 2, num_flat[1], width, label="Numerical", alpha=0.8, color="red"
    )
    ...
    for i, comp in enumerate(components):
        ...
        ax_comp2.text(
            i,
            max(theo_flat[1, i], num_flat[1, i]) + 0.01,
            f"Δ={diff_val:.2e}",
            ha="center",
            fontsize=12,
            color="red",
    ...
    ax_title2.text(
        0.5,
        0.8,
        "QUATERNION PSEUDOINVERSE PERFORMANCE ANALYSIS",
        fontsize=24,
        fontweight="bold",
        ha="center",
    )
    ax_title2.text(
        0.5, 0.6, "Algorithm Benchmarking Across Matrix Types", fontsize=16, ha="center"
    )
    ax_title2.text(
        0.5,
        0.4,
        "Performance Comparison: Dense, Sparse, Ill-conditioned, and Validation Matrices",
        fontsize=14,
        ha="center",
    )
    ...
    bars = ax_iter.bar(
        matrix_types, iterations, color=["blue", "green", "red", "purple"], alpha=0.8
    )
    ...
    bars = ax_time.bar(
        matrix_types, times, color=["orange", "lightblue", "pink", "yellow"], alpha=0.8
    )
...
def main() -> None:
    ...
    print(
        f"A_dense: type={type(A_dense)}, dtype={getattr(A_dense, 'dtype', 'N/A')}, shape={getattr(A_dense, 'shape', 'N/A')}"
    )
    ...
    print(
        f"A_ill: type={type(A_ill)}, dtype={getattr(A_ill, 'dtype', 'N/A')}, shape={getattr(A_ill, 'shape', 'N/A')}"
    )
    ...
    solver = NewtonSchulzPseudoinverse(
        gamma=0.95, max_iter=100, tol=1e-6, verbose=True, compute_residuals=True
    )
    ...
    print(
        f"✓ Dense matrix: {dense_iters} iters, {dense_time:.4f}s, Final cov dev: {cov_dense[-1]:.2e}"
    )
    ...
    print(
        f"✓ Sparse matrix: {sparse_iters} iters, {sparse_time:.4f}s, Final cov dev: {cov_sparse[-1]:.2e}"
    )
    ...
    print(
        f"✓ Ill-conditioned matrix: {ill_iters} iters, {ill_time:.4f}s, Final cov dev: {cov_ill[-1]:.2e}"
    )
    ...
    print(
        f"✓ Validation matrix: {small_iters} iters, {small_time:.4f}s, Final cov dev: {cov_small[-1]:.2e}"
    )
    ...
    print("=" * 80)
    print(
        "Comparing our numerical result with the theoretical pseudoinverse from Example 5.2:"
    )
    print()
    print("Theoretical A^† (from paper):")
    print(
        "⎛ 47/347 + 21/694 i + 11/694 j +  0 k    -21/694  - 11/347 i +  0 j -  11/694 k ⎞"
    )
    print(
        "⎜ -63/347 - 28/347 i + 21/694 j -101/694 k                  61/694 + 21/694 i - 6/347 j + 21/347 k  ⎟"
    )
    print(
        "⎝  57/347 + 49/694 i + 77/694 k                21/347 - 21/694 i - 33/694 k            ⎠"
    )
    ...
    print()
    print(
        "✓ Validation test PASSED" if cov_small[-1] < 1e-5 else "✗ Validation test FAILED"
    )
