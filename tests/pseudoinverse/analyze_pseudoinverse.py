...
def visualize_pseudoinverse(X, X_pinv, original_shape):
    ...
    for i, (color, cmap) in enumerate([('Red', 'Reds'), ('Green', 'Greens'), ('Blue', 'Blues')]):
        plt.subplot(1, 3, i+1)
        plt.imshow(kernel_rgb[:, i].reshape(1, -1), cmap=cmap, aspect='auto', vmin=-1, vmax=1)
    ...
    for i, (name, interpretation) in enumerate(zip(component_names, interpretations)):
        row, col = i // 2, i % 2
        axes[row, col].hist(X_components[:, :, i].flatten(), bins=50, alpha=0.7, 
                           label='Original', color='blue', density=True)
        axes[row, col].hist(X_pinv_components[:, :, i].flatten(), bins=50, alpha=0.7, 
                           label='Pseudoinverse', color='red', density=True)
