...
from solver import QGMRESSolver, NewtonSchulzPseudoinverse
...
def test_qgmres_convergence():
    ...
    for size in sizes:
        ...
        assert solution_diff < tolerance, f"Solution difference too large for size {size}: {solution_diff}"
        assert residual_qgmres < 1e-3, f"Q-GMRES residual too large for size {size}: {residual_qgmres}"
...
def plot_convergence_results(results):
    ...
    plt.tight_layout()
    plt.savefig('../../output_figures/qgmres_convergence_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
