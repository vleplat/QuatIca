...
def analyze_lu_decomposition_accuracy(A: np.ndarray):
    ...
    try:
        ...
        reconstruction_error = quat_frobenius_norm(A - A_reconstructed) / quat_frobenius_norm(A)
...
def solve_with_detailed_tracking(A: np.ndarray, b: np.ndarray, x_true: np.ndarray, method: str):
    ...
    try:
        ...
        solver = QGMRESSolver(tol=1e-10, max_iter=200, verbose=False, preconditioner=method)
...
def run_accuracy_investigation():
    ...
    for size in matrix_sizes:
        ...
        for cond_num in condition_numbers:
            ...
            # Create test matrix
            A, b, x_true, eigenvals = create_controlled_test_matrix(size, cond_num, seed=42)
            
            # Analyze LU decomposition quality
            lu_analysis = analyze_lu_decomposition_accuracy(A)
            print(f"    🔧 LU reconstruction error: {lu_analysis.get('reconstruction_error', 'N/A'):.2e}")
            
            # Test both methods
            for method in ['none', 'left_lu']:
                print(f"    {'🔄' if method == 'none' else '⚡'} Method: {method:8s}", end="")
                ...
                if result['success']:
                    print(f" ✅ {result['iterations']:3d}it, "
...
def create_accuracy_investigation_plot(results: List, output_dir: Path):
    ...
    for method in ['none', 'left_lu']:
        ...
        if method_results:
            ...
            ax1.loglog(cond_nums, errors, 'o-', label=f'Q-GMRES ({method})', 
                      color=colors[method], linewidth=3, markersize=10)
    ...
    ax1.plot(cond_range, ideal_error, '--', color='gray', alpha=0.7, 
    ...
    for method in ['none', 'left_lu']:
        ...
        if method_results:
            ...
            ax2.loglog(reported_residuals, final_residuals, 'o', 
                      label=f'Q-GMRES ({method})', color=colors[method], 
                      markersize=8, alpha=0.7)
    
    # Add perfect agreement line
    res_range = np.logspace(-12, -6, 100)
    ax2.plot(res_range, res_range, '--', color='black', alpha=0.5, label='Perfect agreement')
    ...
    if lu_errors:
        ax3.loglog(cond_nums_lu, lu_errors, 'ro-', linewidth=2, markersize=8,
                  label='LU reconstruction error')
        
        # Add machine precision line
        ax3.axhline(y=1e-16, color='gray', linestyle='--', alpha=0.7, 
    ...
    for cond_num in set(r['condition_number'] for r in successful_results):
        baseline_results = [r for r in successful_results 
                           if r['method'] == 'none' and r['condition_number'] == cond_num]
        precond_results = [r for r in successful_results 
    ...
    if error_ratios:
        ax4.semilogx(cond_nums_ratio, error_ratios, 'go-', linewidth=2, markersize=8,
    ...
    for method in ['none', 'left_lu']:
        ...
        if method_results:
            ...
            ax5.semilogx(cond_nums, iterations, 'o-', label=f'Q-GMRES ({method})', 
    ...
    baseline_errors = [r['solution_error'] for r in successful_results if r['method'] == 'none']
    precond_errors = [r['solution_error'] for r in successful_results if r['method'] == 'left_lu']
    
    if baseline_errors and precond_errors:
        baseline_median = np.median(baseline_errors)
        precond_median = np.median(precond_errors)
        error_ratio_median = precond_median / baseline_median if baseline_median > 0 else float('inf')
        ...
        analysis_text = [
            ...
            f"   • Average LU reconstruction error: {np.mean(lu_errors):.2e}" if lu_errors else "   • LU analysis failed",
            "",
            "💡 POTENTIAL CAUSES:",
    ...
    for r in successful_results:
        if (r['method'] == 'left_lu' and r['condition_number'] == 1000.0 and 
            'x_solution' in r and r['solution_error'] > 1e-10):
    ...
    if representative_case:
        ...
        baseline_case = None
        for r in successful_results:
            if (r['method'] == 'none' and 
                r['condition_number'] == representative_case['condition_number'] and
                r['matrix_size'] == representative_case['matrix_size']):
                baseline_case = r
                break
        ...
        if baseline_case and 'x_solution' in baseline_case:
            ...
            ax7.plot(indices, [x.real for x in x_baseline], 'o-', 
                    label='Baseline solution (real part)', color=colors['none'], linewidth=2)
            ax7.plot(indices, [x.real for x in x_precond], 's-', 
                    label='Preconditioned solution (real part)', color=colors['left_lu'], linewidth=2)
            
            ax7.set_xlabel('Solution Component Index')
            ax7.set_ylabel('Real Part of Solution')
            ax7.set_title(f'Solution Comparison (κ={representative_case["condition_number"]:.0e})', 
                         fontweight='bold')
            ax7.legend()
            ax7.grid(True, alpha=0.3)
    else:
        ax7.text(0.5, 0.5, 'No suitable case found\nfor solution comparison', 
                ha='center', va='center', transform=ax7.transAxes, fontsize=12)
    
    # Overall title
    fig.suptitle('Q-GMRES LU Preconditioning: Accuracy Investigation Report', 
    ...
    plt.savefig(output_dir / 'qgmres_accuracy_investigation.png', 
...
def test_qgmres_accuracy_investigation(dummy):
    ...
    print(f"✅ Saved: qgmres_accuracy_investigation.png")
    ...
    if successful_results:
        ...
        if baseline_errors and precond_errors:
            ...
            if error_ratio > 2.0:
                print(f"   ⚠️  LU preconditioning degrades accuracy significantly!")
            elif error_ratio > 1.1:
                print(f"   ⚠️  LU preconditioning slightly degrades accuracy")
            else:
                print(f"   ✅ LU preconditioning maintains accuracy")
