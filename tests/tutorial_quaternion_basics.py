...
from utils import SparseQuaternionMatrix, quat_frobenius_norm, quat_matmat
...
def visualize_quaternion_matrix(A, name="Matrix", save_plot=True):
    ...
    if isinstance(A, SparseQuaternionMatrix):
        A = quaternion.as_quat_array(
            np.stack(
                [A.real.toarray(), A.i.toarray(), A.j.toarray(), A.k.toarray()], axis=-1
            )
        )
    ...
    fig.suptitle(
        f"Quaternion Matrix: {name} (shape: {A.shape})", fontsize=16, fontweight="bold"
    )
    ...
    if save_plot:
        ...
        plt.savefig(
            f"output_figures/{name.replace(' ', '_').lower()}_visualization.png",
            dpi=300,
            bbox_inches="tight",
        )
    plt.show()
...
def plot_convergence(
    residuals, covariances, title="Newton-Schulz Convergence", save_plot=True
):
    ...
    ax1.semilogy(
        range(1, len(residuals["AX-herm"]) + 1),
        residuals["AX-herm"],
        "^-",
        label="AX-herm",
        linewidth=2,
    ...
    ax1.semilogy(
        range(1, len(residuals["XA-herm"]) + 1),
        residuals["XA-herm"],
        "d-",
        label="XA-herm",
        linewidth=2,
    ...
    ax2.semilogy(
        range(1, len(covariances) + 1),
        covariances,
        "o-",
        color="red",
        linewidth=2,
        markersize=6,
        label="Covariance Deviation",
    )
    ax2.set_xlabel("Iteration", fontsize=12)
    ax2.set_ylabel("Covariance Deviation", fontsize=12)
    ax2.set_title(
        "Covariance Deviation ||AX-I|| or ||XA-I||", fontsize=14, fontweight="bold"
    )
    ...
    if save_plot:
        ...
        os.makedirs("output_figures", exist_ok=True)
        plt.savefig(
            f"output_figures/{title.replace(' ', '_').lower()}_convergence.png",
            dpi=300,
            bbox_inches="tight",
        )
    plt.show()
...
def plot_performance_comparison(
    sizes, times, iterations, title="Performance Scaling", save_plot=True
):
    ...
    if save_plot:
        ...
        os.makedirs("output_figures", exist_ok=True)
        plt.savefig(
            f"output_figures/{title.replace(' ', '_').lower()}_performance.png",
            dpi=300,
            bbox_inches="tight",
        )
    plt.show()
...
def main():
    ...
    print(
        f"   ✓ Solution verification: residual is {'small' if residual_norm < 1e-6 else 'large'}"
    )
    ...
    plot_performance_comparison(
        sizes, times, iterations, "Quaternion Pseudoinverse Performance"
    )
    ...
    print(
        "4. Use quat_matmat() for matrix-vector multiplication (vectors are matrices with one column)"
    )
    ...
    def create_summary_visualization():
        ...
        ax.text(
            5,
            7,
            "Quaternion Matrix Operations Complete!",
            fontsize=14,
            ha="center",
            color="darkgreen",
        )
        ...
        for feature, x, y, color in features:
            ...
            ax.add_patch(rect)
            ax.text(
                x, y, feature, fontsize=10, fontweight="bold", ha="center", va="center"
            )
