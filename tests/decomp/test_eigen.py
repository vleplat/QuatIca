...
import numpy as np
import quaternion
from core.decomp.eigen import (
    quaternion_eigendecomposition, quaternion_eigenvalues, 
    quaternion_eigenvectors, verify_eigendecomposition
)
...
class TestEigenvalueDecomposition(unittest.TestCase):
    ...
    def test_reconstruction_from_eigenpairs(self):
        ...
        for i in range(3):
            for j in range(3):
                if i == j:
                    D_quat[i, j] = quaternion.quaternion(eigenvalues[i].real, eigenvalues[i].imag, 0, 0)
