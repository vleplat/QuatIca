...
from decomp.qsvd import rand_qsvd, pass_eff_qsvd, classical_qsvd_full
...
def create_performance_plots(results, full_time, n_passes_list):
    ...
    fig.suptitle('Q-SVD Performance Comparison: rand_qsvd vs pass_eff_qsvd\n(500×300 matrix, rank=10)', 
                 fontsize=16, fontweight='bold')
    
    # Plot 1: Execution Time
    ax1.plot(n_passes_list[:len(rand_times)], rand_times, 'o-', linewidth=2, markersize=8, 
             label='rand_qsvd', color='blue')
    ax1.plot(n_passes_list[:len(pass_times)], pass_times, 's-', linewidth=2, markersize=8, 
             label='pass_eff_qsvd', color='red')
    ax1.axhline(y=full_time, color='green', linestyle='--', linewidth=2, 
                label='Full Q-SVD', alpha=0.7)
    ...
    ax2.plot(n_passes_list[:len(rand_speedups)], rand_speedups, 'o-', linewidth=2, markersize=8, 
             label='rand_qsvd', color='blue')
    ax2.plot(n_passes_list[:len(pass_speedups)], pass_speedups, 's-', linewidth=2, markersize=8, 
             label='pass_eff_qsvd', color='red')
    ax2.axhline(y=1.0, color='green', linestyle='--', linewidth=2, 
                label='Full Q-SVD baseline', alpha=0.7)
    ...
    ax3.semilogy(n_passes_list[:len(rand_errors)], rand_errors, 'o-', linewidth=2, markersize=8, 
                 label='rand_qsvd', color='blue')
    ax3.semilogy(n_passes_list[:len(pass_errors)], pass_errors, 's-', linewidth=2, markersize=8, 
                 label='pass_eff_qsvd', color='red')
    ax3.axhline(y=1e-15, color='green', linestyle='--', linewidth=2, 
    ...
    ax4.scatter(rand_errors, rand_speedups, s=100, c='blue', alpha=0.7, 
                label='rand_qsvd', edgecolors='black')
    ax4.scatter(pass_errors, pass_speedups, s=100, c='red', alpha=0.7, 
                label='pass_eff_qsvd', edgecolors='black')
    
    # Add labels for each point
    for i, (error, speedup) in enumerate(zip(rand_errors, rand_speedups)):
        ax4.annotate(f'{n_passes_list[i]}', (error, speedup), 
...
def benchmark_qsvd_methods():
    ...
    for n_passes in n_passes_list:
        ...
        start_time = time.time()
        try:
            U_rand, s_rand, V_rand = rand_qsvd(X, target_rank, oversample=oversample, n_iter=n_passes)
        ...
        start_time = time.time()
        try:
            U_pass, s_pass, V_pass = pass_eff_qsvd(X, target_rank, oversample=oversample, n_passes=n_passes)
    ...
    # Print header
    print(f"{'Passes':<8} {'rand_qsvd Time':<15} {'rand Speedup':<12} {'rand Rel Error':<12} {'rand SVD Diff':<12}")
    print(f"{'':<8} {'pass_eff_qsvd Time':<15} {'pass Speedup':<12} {'pass Rel Error':<12} {'pass SVD Diff':<12}")
    print("-" * 80)
    
    for result in results:
        # rand_qsvd results
        rand_time_str = f"{result['rand_time']:.3f}s" if result['rand_success'] else "FAILED"
        rand_speedup_str = f"{full_time/result['rand_time']:.1f}x" if result['rand_success'] else "N/A"
        rand_error_str = f"{result['rand_rel_error']:.6f}" if result['rand_success'] else "N/A"
        rand_svd_str = f"{result['rand_s_diff']:.6f}" if result['rand_success'] else "N/A"
        
        # pass_eff_qsvd results
        pass_time_str = f"{result['pass_time']:.3f}s" if result['pass_success'] else "FAILED"
        pass_speedup_str = f"{full_time/result['pass_time']:.1f}x" if result['pass_success'] else "N/A"
        pass_error_str = f"{result['pass_rel_error']:.6f}" if result['pass_success'] else "N/A"
        pass_svd_str = f"{result['pass_s_diff']:.6f}" if result['pass_success'] else "N/A"
        
        print(f"{result['n_passes']:<8} {rand_time_str:<15} {rand_speedup_str:<12} {rand_error_str:<12} {rand_svd_str:<12}")
        print(f"{'':<8} {pass_time_str:<15} {pass_speedup_str:<12} {pass_error_str:<12} {pass_svd_str:<12}")
        print()
    ...
    if successful_results:
        ...
        print(f"  rand_qsvd: {best_accuracy_rand['rand_rel_error']:.6f} (n_iter={best_accuracy_rand['n_passes']})")
        print(f"  pass_eff_qsvd: {best_accuracy_pass['pass_rel_error']:.6f} (n_passes={best_accuracy_pass['n_passes']})")
        ...
        print(f"\nBest speed:")
        print(f"  rand_qsvd: {best_speed_rand['rand_time']:.3f}s (n_iter={best_speed_rand['n_passes']})")
        print(f"  pass_eff_qsvd: {best_speed_pass['pass_time']:.3f}s (n_passes={best_speed_pass['n_passes']})")
