...
import numpy as np
import quaternion
from core.decomp.tridiagonalize import (
    tridiagonalize, householder_matrix, householder_vector, 
    internal_tridiagonalizer, check_tridiagonal
)
        ...
        for i in range(2):
            for j in range(2):
                if abs(i - j) > 1:  # Off-tridiagonal elements
                    element = B[i, j]
                    magnitude = np.sqrt(element.w**2 + element.x**2 + element.y**2 + element.z**2)
