...
from decomp.qsvd import rand_qsvd, classical_qsvd_full
from utils import quat_frobenius_norm, quat_matmat, quat_hermitian
...
def test_rand_qsvd_basic():
    ...
    for n_iter, oversample in test_cases:
        ...
        try:
            ...
            print(f"  ✅ SUCCESS: rand_qsvd completed")
...
def test_power_iteration_convergence():
    ...
    if results:
        ...
        for result in results:
            print(f"{result['n_iter']:6d} | {result['s_diff']:7.4f} | {result['s_relative_diff']:10.4f} | "
        ...
        if len(results) > 1:
            ...
            if last_error < first_error:
                print("✅ CONVERGENCE OBSERVED: Higher power iterations improve accuracy!")
            else:
