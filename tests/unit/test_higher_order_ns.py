...
class TestHigherOrderNS(unittest.TestCase):
    def test_compare_residuals_and_time(self):
        ...
        for (m, n) in sizes:
            ...
            # Baseline NS (damped)
            ns = NewtonSchulzPseudoinverse(gamma=0.5, max_iter=60, tol=1e-10, verbose=False, compute_residuals=True)
            t0 = time.time()
            X_ns, res_ns, times_ns = ns.compute(A)
            t_ns = time.time() - t0

            # Higher-order NS (third-order)
            hon = HigherOrderNewtonSchulzPseudoinverse(max_iter=60, tol=0.0, verbose=False)
            t1 = time.time()
            ...
            print(f"Size {m}x{n}: NS E1={e1_ns:.2e} time={t_ns:.2f}s | HON E1={e1_hon:.2e} time={t_hon:.2f}s")
