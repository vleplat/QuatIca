...
from utils import (quat_null_space, quat_null_right, quat_null_left, quat_kernel,
                   quat_matmat, quat_hermitian, quat_frobenius_norm, quat_eye)
...
class TestKernelNullSpace(unittest.TestCase):
    ...
    def _create_known_null_space_matrix(self) -> tuple:
        ...
        A = np.array([
            [quaternion.quaternion(1, 0, 0, 0), quaternion.quaternion(0, 1, 0, 0), quaternion.quaternion(0, 0, 1, 0)],
            [quaternion.quaternion(0, 0, 0, 1), quaternion.quaternion(1, 0, 0, 0), quaternion.quaternion(0, 1, 0, 0)],
            [quaternion.quaternion(0, 0, 1, 0), quaternion.quaternion(0, 0, 0, 1), quaternion.quaternion(1, 0, 0, 0)],
            [quaternion.quaternion(0, 1, 0, 0), quaternion.quaternion(0, 0, 1, 0), quaternion.quaternion(0, 0, 0, 1)]
        ])
        
        # Create B such that last 3 columns have specific structure
        B = np.array([
            [quaternion.quaternion(1, 0, 0, 0), quaternion.quaternion(0, 1, 0, 0), quaternion.quaternion(0, 0, 1, 0),
             quaternion.quaternion(1, 1, 0, 0), quaternion.quaternion(0, 0, 0, 0), quaternion.quaternion(0, 0, 0, 0)],
            [quaternion.quaternion(0, 0, 0, 1), quaternion.quaternion(1, 0, 0, 0), quaternion.quaternion(0, 1, 0, 0),
             quaternion.quaternion(0, 0, 1, 1), quaternion.quaternion(0, 0, 0, 0), quaternion.quaternion(0, 0, 0, 0)],
            [quaternion.quaternion(0, 0, 1, 0), quaternion.quaternion(0, 0, 0, 1), quaternion.quaternion(1, 0, 0, 0),
             quaternion.quaternion(1, 0, 0, 1), quaternion.quaternion(0, 0, 0, 0), quaternion.quaternion(0, 0, 0, 0)]
        ])
    ...
    def test_quat_null_space_orthogonality_right(self):
        ...
        if N.shape[1] > 0:  # If null space is non-empty
            ...
            self.assertLess(AN_norm, self.tol, 
                           f"A @ N should be zero, got norm {AN_norm}")
    ...
    def test_quat_null_space_orthogonality_left(self):
        ...
        if N.shape[1] > 0:  # If null space is non-empty
            ...
            self.assertLess(norm, self.tol,
                           f"N^H @ A should be zero, got norm {norm}")
    ...
    def test_quat_null_space_known_example(self):
        ...
        A = np.array([
            [quaternion.quaternion(1, 0, 0, 0), quaternion.quaternion(2, 0, 0, 0), quaternion.quaternion(3, 0, 0, 0)],
            [quaternion.quaternion(0, 1, 0, 0), quaternion.quaternion(0, 2, 0, 0), quaternion.quaternion(0, 3, 0, 0)]
        ])
    ...
    def test_consistency_across_different_ranks(self):
        ...
        for rank in range(0, min(m, n) + 1):
            with self.subTest(rank=rank):
                ...
                self.assertLessEqual(abs(N_right.shape[1] - expected_right_dim), 1,
                ...
                self.assertLessEqual(abs(N_left.shape[1] - expected_left_dim), 1,
...
if __name__ == '__main__':
    ...
    # Print summary
    print(f"\n{'='*60}")
    print(f"KERNEL/NULL SPACE TESTS SUMMARY")
    print(f"{'='*60}")
