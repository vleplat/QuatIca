...
def plot_reconstruction_error_vs_rank():
    ...
    for idx, (m, n, title) in enumerate(test_cases):
        ...
        ax.plot(ranks, reconstruction_errors, 'bo-', linewidth=2, markersize=8, label='Reconstruction Error')
        ...
        for i, (r, err) in enumerate(zip(ranks, reconstruction_errors)):
            if i == 0:  # First point
                ax.annotate(f'{err:.3f}', (r, err), textcoords="offset points", 
        ...
        ax2 = ax.twinx()
        ax2.plot(ranks, relative_errors, 'ro--', linewidth=1, markersize=4, alpha=0.7, label='Relative Error')
        ...
        # Verify monotonicity
        is_monotonic = all(reconstruction_errors[i] <= reconstruction_errors[i-1] 
        ...
        print(f"  Perfect reconstruction at full rank: {'✅ PASSED' if perfect_reconstruction else '❌ FAILED'}")
...
def plot_relative_error_summary():
    ...
    plt.axhline(y=1e-15, color='gray', linestyle='--', alpha=0.5, label='Machine Precision')
...
def generate_validation_report():
    ...
    # Run the analysis
    fig1 = plot_reconstruction_error_vs_rank()
    fig2 = plot_relative_error_summary()
