...
def run_case(m: int, n: int, max_iter: int = 80):
    ...
    # NS damped (gamma=0.5)
    ns_damped = NewtonSchulzPseudoinverse(
        gamma=0.5, max_iter=max_iter, tol=1e-12, verbose=False, compute_residuals=True
    )
    t0 = time.time()
    X_ns_d, res_ns_d, times_ns_d = ns_damped.compute(A)
    t_ns_d = time.time() - t0

    # NS baseline (gamma=1)
    ns_unit = NewtonSchulzPseudoinverse(
        gamma=1.0, max_iter=max_iter, tol=1e-12, verbose=False, compute_residuals=True
    )
    t1 = time.time()
    X_ns_u, res_ns_u, times_ns_u = ns_unit.compute(A)
    t_ns_u = time.time() - t1
    ...
    return (
        (res_ns_d["AXA-A"], t_ns_d),
        (res_ns_u["AXA-A"], t_ns_u),
        (res_hon["AXA-A"], t_hon),
    )
...
def visualize(results, sizes, out_dir="output_figures"):
    ...
    for (m, n), (nsd_res, nsd_time), (nsu_res, nsu_time), (hon_res, hon_time) in zip(
...
if __name__ == "__main__":
    ...
    hon_results = []
    for m, n in sizes:
        (nsd_res, nsd_time), (nsu_res, nsu_time), (hon_res, hon_time) = run_case(
            m, n, max_iter=80
        )
