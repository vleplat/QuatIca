...
def test_dieudonne_determinant_svd_decomposition():
    ...
    for m, n in test_cases:
        ...
        print(f"Constructed matrix A = U @ S @ V^H")
        ...
        if relative_error < tolerance:
            print(f"✓ SUCCESS: Relative error {relative_error:.2e} < tolerance {tolerance}")
        else:
            print(f"✗ FAILED: Relative error {relative_error:.2e} >= tolerance {tolerance}")
        ...
        U_unitarity_error = np.linalg.norm(quaternion.as_float_array(U_H_U - np.eye(m, dtype=np.quaternion)))
        V_unitarity_error = np.linalg.norm(quaternion.as_float_array(V_H_V - np.eye(n, dtype=np.quaternion)))
