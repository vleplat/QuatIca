...
def test_qgmres_lu_preconditioner(n: int, seed: int):
    ...
    gmres_lu = QGMRESSolver(
        tol=1e-6, max_iter=2 * n, verbose=False, preconditioner="left_lu"
    )
    ...
    print(
        f"n={n} seed={seed} | iters: none={info0['iterations']} lu={info1['iterations']} | res: none={r0:.3e} lu={r1:.3e}"
    )
