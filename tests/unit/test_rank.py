...
This module tests the rank function from core/utils.py to ensure it correctly
...
from utils import rank, quat_matmat
    ...
    def test_rank_matrix_product(self):
        ...
        for m, r, n in test_cases:
            with self.subTest(m=m, r=r, n=n):
                ...
                # Check if they match
                self.assertEqual(computed_rank, expected_rank,
                               f"Size {m}×{r} @ {r}×{n}: Expected rank {expected_rank}, got {computed_rank}")
                
                print(f"✓ Size {m}×{r} @ {r}×{n}: rank(C) = {computed_rank} (expected: {expected_rank})")
    ...
    def test_rank_full_rank_matrix(self):
        ...
        for m, n in sizes:
            with self.subTest(m=m, n=n):
                ...
                print(f"✓ Size {m}×{n}: rank(A) = {computed_rank} (expected: {expected_rank})")
    ...
    def test_rank_low_rank_matrix(self):
        ...
        for m, n, target_rank in test_cases:
            with self.subTest(m=m, n=n, target_rank=target_rank):
                ...
                self.assertEqual(computed_rank, target_rank,
                               f"Size {m}×{n}, target rank {target_rank}: Got {computed_rank}")
                
                print(f"✓ Size {m}×{n}, target rank {target_rank}: rank(C) = {computed_rank}")
