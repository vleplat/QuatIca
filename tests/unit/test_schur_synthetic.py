...
def build_upper_triangular_quat(
    diag_vals: np.ndarray,
    super_scale: float = 0.1,
...
def build_upper_triangular_quat_full(
    diag_vals: np.ndarray,
    super_scale: float = 0.1,
    rng: np.random.Generator | None = None,
) -> np.ndarray:
...
def test_schur_synthetic_diagonalizable(n: int, seed: int, variant: str):
    ...
    save_T_magnitude(
        T,
        title=f"|T| (diag case) variant={variant}, n={n}, seed={seed}",
        fname=f"schur_T_abs_diag_variant-{variant}_n{n}_seed{seed}.png",
    )
...
def test_schur_synthetic_upper_triangular(n: int, seed: int, variant: str):
    ...
    save_T_magnitude(
        T,
        title=f"|T| (upper-tri case) variant={variant}, n={n}, seed={seed}",
        fname=f"schur_T_abs_upper_variant-{variant}_n{n}_seed{seed}.png",
    )
...
def test_schur_synthetic_upper_triangular_full_quat(n: int, seed: int, variant: str):
    ...
    save_T_magnitude(
        T,
        title=f"|T| (upper-tri full-quat) variant={variant}, n={n}, seed={seed}",
        fname=f"schur_T_abs_upper_full_variant-{variant}_n{n}_seed{seed}.png",
    )
